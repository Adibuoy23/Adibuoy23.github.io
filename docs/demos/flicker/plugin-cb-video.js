var jsPsychCbVideo = (function (jspsych) {
  ...
  /* Blog copy: timers inside this plugin are pausable, so the demo can be
     paused and resumed from the page. Shadows the global setTimeout and
     clearTimeout for the rest of this file only. */
  const __timers = new Map(); let __nextId = 1; let __paused = false;
  function setTimeout(cb, ms) {
    const id = __nextId++;
    const t = { cb: cb, remaining: ms, start: performance.now(), native: null };
    if (!__paused) t.native = window.setTimeout(function () { __timers.delete(id); cb(); }, ms);
    __timers.set(id, t);
    return id;
  }
  function clearTimeout(id) {
    const t = __timers.get(id);
    if (t) { window.clearTimeout(t.native); __timers.delete(id); }
  }
  window.__flickerPause = function () {
    if (__paused) return; __paused = true;
    const now = performance.now();
    __timers.forEach(function (t) { window.clearTimeout(t.native); t.remaining -= (now - t.start); });
    document.querySelectorAll('video').forEach(function (v) { v.pause(); });
  };
  window.__flickerResume = function () {
    if (!__paused) return; __paused = false;
    const now = performance.now();
    __timers.forEach(function (t, id) {
      t.start = now;
      t.native = window.setTimeout(function () { __timers.delete(id); t.cb(); }, Math.max(0, t.remaining));
    });
